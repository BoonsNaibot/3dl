...
class WeakList(list):
    ...
    def __contains__(self, item):
        return super(WeakList, self).__contains__(self._get_ref(item))
    ...
    def __setitem__(self, i, item):
        super(WeakList, self).__setitem__(i, self._get_ref(item))

    def __setslice__(self, i, j, items):
        super(WeakList, self).__setslice__(i, j, (self._get_ref(x) for x in items))
    ...
    def append(self, item):
        super(WeakList, self).append(self._get_ref(item))

    def count(self, item):
        return super(WeakList, self).count(self._get_ref(item))

    def extend(self, items):
        super(WeakList, self).extend((self._get_ref(x) for x in items))

    def index(self, item):
        return super(WeakList, self).index(self._get_ref(item))
